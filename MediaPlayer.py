...
import Out_file
from bot import warning
import running_model_script as rms
...
class InfoDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setGeometry(1200, 210, 500, 300)

        # Создание окна вывода текста (QTextEdit)
        self.output_text_edit = QTextEdit(self)
        font = QFont("Arial", 30)  # Change the font and size as needed
        self.output_text_edit.setFont(font)
        self.output_text_edit.setAlignment(Qt.AlignCenter)
        ...
        # layout.addWidget(self.image_label, 2)  # Окно вывода изображения занимает 2/3 диалога
        layout.addWidget(self.output_text_edit)   # Окно ввода занимает 1/3 диалога
        # layout.addWidget(self.save_button)
        layout.addWidget(self.close_button)

    def save_info(self, text):
        # Путь к файлу content.txt
        print(type(text))
        print(text)

        output_text_file_path = os.path.join(os.path.dirname(__file__), "content", "report.txt")
        ...
        QMessageBox.information(self, "Сохранено", f"Информация сохранена успешно в {output_text_file_path}")
...
class Window(QMainWindow):
    def __init__(self, parent=None, border=None):
        ...
        self.setWindowIcon(QIcon('icons/123.ico'))
    ...
    def closeEvent(self, event):
        # Здесь ваш код, который будет выполняться при закрытии окна
        print("Окно закрывается")
        with open("content/report.txt", "w") as file:
            pass
        QApplication.quit()
        # Не забудьте вызвать closeEvent родительского класса
    ...
    def ui_init(self):
        ...
        self.p_play.clicked.connect(self.ply_playlist)
        ...
        self.find.clicked.connect(self.process_current_video)
    ...
    def load_url(self):
        ...
        # Добавление каждого файла в плейлист
        # for video_file in video_files:
        #     file_path = os.path.join(folder_path, video_file)
        #     self.playlist.addMedia(QMediaContent(QUrl.fromLocalFile(file_path)))

        # Сигнал о изменении макета (если это необходимо для вашего приложения)
        self.model.layoutChanged.emit()

        # Очистка поля ввода URL
        self.url_input.clear()

        # Запуск таймера (если это необходимо для вашего приложения)
    ...
    def play_playlist_item(self):
        ...
        # Проверяем, если воспроизведение дошло до конца плейлиста
        if self.playlist.currentIndex() == self.playlist.mediaCount() - 1:
            # В этом случае не запускаем следующее видео
            return
    ...
    def process_current_video(self):
        current_media = self.mediaPlayer.currentMedia()
        if current_media.isNull():
            # Нет текущего элемента в плейлисте
            return
        #info_dialog = InfoDialog(self)
        current_file_path = current_media.canonicalUrl().toLocalFile()
        # Теперь у вас есть путь к текущему видеофайлу
        file_name = ''
        file_name += os.path.basename(current_file_path)
        print(current_file_path)
        # Вы можете передать его в вашу функцию для дополнительной обработки
        text = rms.predict(current_file_path)
        #info_dialog.save_info(text)
    ...
    def show_info_dialog(self):
        # Проверяем, существует ли атрибут класса 'current_image_index'
        if not hasattr(InfoDialog, 'current_image_index'):
            # Если нет, создаем его и устанавливаем в 0
            InfoDialog.current_image_index = 0

        # Определяем список путей к изображениям
        image_paths = ['0018.jpg', '0030.jpg', '0031.jpg']  # Замените второй путь на свой

        # Получаем путь к текущему изображению на основе текущего индекса
        image_path = image_paths[InfoDialog.current_image_index]

        # Создаем экземпляр InfoDialog внутри функции
        info_dialog = InfoDialog(self)

        # Загружаем изображение с использованием QPixmap
        pixmap = QPixmap(image_path)

        # Устанавливаем загруженное изображение в QLabel в InfoDialog
        # info_dialog.image_label.setPixmap(pixmap)

        # Увеличиваем текущий индекс для следующего вызова
        InfoDialog.current_image_index = (InfoDialog.current_image_index + 1) % len(image_paths)
